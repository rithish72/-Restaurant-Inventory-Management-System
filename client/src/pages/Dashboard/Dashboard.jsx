    ...
    <div className="container-db">
      <div className={`dashboard-container ${darkMode ? "dark-bg-db" : "light-bg-db"} animate-in`}>
        ...
        <div className="row animate-in">
          {[
            { title: 'Total Inventory Items', value: totalItems, bg: 'bg-success' },
            { title: 'Low Stock Items', value: lowStockItems.length, bg: 'bg-warning' }
          ].map((card, i) => (
            <div className="col-md-4 animate-in" key={i}>
              <div className={`card text-white ${card.bg} mb-3`}>
                <div className="card-body">
                  <h5 className="card-title">{card.title}</h5>
                  <p className="card-text fs-2">{card.value}</p>
                </div>
              </div>
            </div>
          ))}
