...
const UpdateItem = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchItem = async () => {
        try {
          const response = await api.get(`/api/v1/inventory/get-inventory-item/${id}`);
  ...
  const handleSubmit = async () => {
    const { itemName, category, quantity, unit, threshold, _id } = updatedItem;
  ...
  return (
    <div className='container-db'>
        <div className={`dashboard-container mt-4 ${darkMode ? 'dark-bg-db' : 'light-bg-db'} animate-in`}>
        ...
        <button className="btn btn-success mt-4 w-100 fw-bold" onClick={handleSubmit}>
